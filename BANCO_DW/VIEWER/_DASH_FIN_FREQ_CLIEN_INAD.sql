...
/****** Object:  View [dbo].[DASH_FIN_FREQ_CLIEN_INAD]    Script Date: 27/06/2024 13:25:15 ******/
SET ANSI_NULLS ON
GO
...
SELECT 
	'Um' = (select count(distinct codigo) from VW_FIN_CLIEN_INAD where mes = month(getdate()) -1 and Dias_Atraso >= 1	and  Dias_Atraso < 50 ),
	'Cinquenta' = (select count(distinct codigo) from VW_FIN_CLIEN_INAD where mes = month(getdate()) -1 and Dias_Atraso >= 50	and  Dias_Atraso < 100 ),
	'Cem' = (select count(distinct codigo) from VW_FIN_CLIEN_INAD where mes = month(getdate()) -1 and Dias_Atraso >= 100	and  Dias_Atraso < 150 ),
	'Cento_50' = (select count(distinct codigo) from VW_FIN_CLIEN_INAD where mes = month(getdate()) -1 and Dias_Atraso >= 150	and  Dias_Atraso < 200 ),
	'Duzentos' = (select count(distinct codigo) from VW_FIN_CLIEN_INAD where mes = month(getdate()) -1 and Dias_Atraso >= 200	and  Dias_Atraso < 250 ),
	'Duzentos_50' = (select count(distinct codigo) from VW_FIN_CLIEN_INAD where mes = month(getdate()) -1 and Dias_Atraso >= 250)
