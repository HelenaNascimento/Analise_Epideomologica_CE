DECLARE @BD_DW int = (SELECT COUNT(NUMERO) FROM R_PDVCB), @BD_REMOTE int = (SELECT COUNT(NUMERO) FROM PROD_2023.dbo.PDVCB)
...
BEGIN TRY
	...
	IF @BD_DW < @BD_REMOTE
	BEGIN
		INSERT INTO R_PDVCB ([Numero], [Cod_PedCli] ,[Tip_Pedido],[Cod_Cliente],[Cod_RegTri],[Cod_Vendedor],[Id_PolCom],[Cod_Prz],[Cod_Rota],[Status1],[Status2],[Dat_Pedido],[Hor_ConferIni],[Hor_ConferFim],[Hor_DbqFin],
		...
		WHERE Cod_Estabe = 1
			AND NOT EXISTS (SELECT * FROM R_PDVCB)
	END
	...
	BEGIN 
		...
		FROM R_PDVCB R_CB
			INNER JOIN PROD_2023.dbo.PDVCB CB ON  R_CB.[Numero] = CB.[Numero]  
		...
		--OR R_CB.[Observacao] <> CB.[Observacao] 
		--OR R_CB.[Obs_Padrao]  <> CB.[Obs_Padrao]  
		--OR R_CB.[Obs_NotFis] <> CB.[Obs_NotFis] 
		--OR R_CB.[Obs_IntFecPdv]  <> CB.[Obs_IntFecPdv]  
		--OR R_CB.[Obs_IntCtaRec] <> CB.[Obs_IntCtaRec] 
