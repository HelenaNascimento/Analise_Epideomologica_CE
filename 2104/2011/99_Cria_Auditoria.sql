...
Declare @BancoAudit Sysname;
Set @BancoAudit = DB_NAME() + '_AUDIT';

If Not Exists (Select 0 From sys.databases Where name = @BancoAudit)
Begin
	...
	Set @Cmd = 'CREATE DATABASE ['+@BancoAudit+']
	 On  PRIMARY 
	( NAME = N'''+@BancoAudit+'_1'', FILENAME = N'''+@Diretorio+@BancoAudit+'_1.mdf'', SIZE = 100MB , FILEGROWTH = 100MB ),
	( NAME = N'''+@BancoAudit+'_2'', FILENAME = N'''+@Diretorio+@BancoAudit+'_2.ndf'', SIZE = 100MB , FILEGROWTH = 100MB ),
	( NAME = N'''+@BancoAudit+'_3'', FILENAME = N'''+@Diretorio+@BancoAudit+'_3.ndf'', SIZE = 100MB , FILEGROWTH = 100MB ),
	( NAME = N'''+@BancoAudit+'_4'', FILENAME = N'''+@Diretorio+@BancoAudit+'_4.ndf'', SIZE = 100MB , FILEGROWTH = 100MB )
	 LOG On 
	( NAME = N'''+@BancoAudit+'_log'', FILENAME = N'''+@Diretorio+@BancoAudit+'_log.ldf'', SIZE = 512MB , FILEGROWTH = 512MB );
	ALTER DATABASE ['+@BancoAudit+'] Set RECOVERY SIMPLE;
	ALTER DATABASE ['+@BancoAudit+'] Set PAGE_VERIFY CHECKSUM;
	ALTER AUTHORIZATION On DATABASE::['+@BancoAudit+'] TO [sa]
	';
	Exec(@Cmd);
End;
Go
...
Set @BancoAudit = DB_NAME() + '_AUDIT';
Declare @Cmd Nvarchar(Max);
Set @Cmd = 'Create Synonym TBAUDNEW FOR ['+@BancoAudit+'].dbo.TBAUDNEW';
...
Set @BancoAudit = DB_NAME() + '_AUDIT';
Declare @Cmd Nvarchar(Max);
Set @Cmd = 'Create Synonym TBAUDNEW_EXC FOR ['+@BancoAudit+'].dbo.TBAUDNEW_EXC';
...
Set @BancoAudit = DB_NAME() + '_AUDIT';
Declare @Cmd Nvarchar(Max);
Set @Cmd = 'Create Synonym TBAUDNEW_SPID FOR ['+@BancoAudit+'].dbo.TBAUDNEW_SPID';
