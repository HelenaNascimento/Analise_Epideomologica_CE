SELECT DISTINCT 
	...
	CASE
		...
		WHEN pdv.Status1 = 'P' and pdv.Status2 = 'N'  THEN 'RECEIVED_BY_DISTRIBUTOR'
		WHEN (pdv.Status1 = 'C' OR (pde.Num_PedVen = 0 AND pdv.Numero IS NULL)) THEN 'CANCELED'
		WHEN (pdv.Status1 = 'D' or pdv.Status1 = 'P')  AND NF.Ret_CStat = 100 AND NF.Dat_Saida IS NULL and
				DATEADD(day, -20, CURRENT_TIMESTAMP) < pdv.Hor_Fatura THEN 'BILLED'
		WHEN pdv.Status1 = 'D' AND NF.Ret_CStat = 100 AND NF.Dat_Saida IS NOT NULL AND
				Dat_RetEnt IS NULL and DATEADD(day, -20, CURRENT_TIMESTAMP) < pdv.Hor_Fatura THEN 'IN_TRANSIT'
		WHEN pdv.Status1 = 'D' AND NF.Ret_CStat = 100 AND NF.Dat_Saida IS NOT NULL AND
				Dat_RetEnt IS NOT NULL THEN 'DELIVERED'
		WHEN pdv.Status1 = 'D' AND NF.Ret_CStat = 100 AND NF.Dat_Saida IS NULL and
				DATEADD(day, -20, CURRENT_TIMESTAMP) >= pdv.Hor_Fatura THEN 'DELIVERED'
		WHEN pdv.Status1 = 'D' AND NF.Ret_CStat = 100 AND NF.Dat_Saida IS NOT NULL AND
				Dat_RetEnt IS NULL and DATEADD(day, -20, CURRENT_TIMESTAMP) >= pdv.Hor_Fatura THEN 'DELIVERED'
		END                      AS 'status',
...
FROM PDECB pde
    ...
    AND NF.Ser_Nota = '1'
--WHERE pde.Cod_PedCli IN ('176073208711142')
GROUP BY pdv.Numero,
            pde.Cod_PedCli,
            pde.Num_PedVen,
            pde.Cod_Client,
