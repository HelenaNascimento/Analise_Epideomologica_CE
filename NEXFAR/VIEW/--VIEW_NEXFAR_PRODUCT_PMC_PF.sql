...
CREATE VIEW VIEW_NEXFAR_PRODUCT_PMC_PF AS
...
SELECT DISTINCT produ.codigo AS "productId",
                ...
                CASE
                    WHEN pcxes.cod_estabe = 0
                         AND COALESCE(produ.prc_fabric205, 0) <> 0 THEN produ.prc_fabric205
                    WHEN pcxes.cod_estabe = 1
                         AND COALESCE(produ.prc_fabric20, 0) <> 0 THEN produ.prc_fabric20
                    WHEN pcxes.cod_estabe = 3
                         AND COALESCE(produ.prc_fabric205, 0) <> 0 THEN produ.prc_fabric205
                    WHEN pcxes.cod_estabe = 4
                         AND COALESCE(produ.prc_fabric19, 0) <> 0 THEN produ.prc_fabric19
                ...
                END AS "pmc"
FROM produ
INNER JOIN pcxpr ON pcxpr.cod_produt = produ.codigo
INNER JOIN pocom ON pcxpr.id_polcom = pocom.id_polcom
INNER JOIN pcxes ON pcxes.id_polcom = pcxpr.id_polcom
INNER JOIN prxes ON pcxpr.cod_produt = prxes.cod_produt
